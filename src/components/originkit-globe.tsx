    ...
    useEffect(() => {
        ...
        const fitCamera = (width: number, height: number) => {
            ...
            const verticalDistance =
                globeRadius / Math.tan((cameraFov * Math.PI) / 360);
            const horizontalFactor = Math.min(1, Math.max(aspect, 0.25));
            const distance = (verticalDistance / horizontalFactor) * 1.2;
            camera.aspect = aspect;
            camera.position.set(0, 0, distance);
            camera.lookAt(0, 0, 0);
            camera.updateProjectionMatrix();
        };
        fitCamera(containerWidth, containerHeight);
        ...
        if (enableDrag) canvas.addEventListener("mousedown", handleMouseDown);
        ...
        return () => {
            ...
            if (enableDrag) canvas.removeEventListener("mousedown", handleMouseDown);
